...
def adb_install():
    app_path = input('>')
    os.system('adb -s %s install %s' % (select_udid, app_path))
...
def run_monkey():
    import setting_monkey
    import monkeyTest
    setting_monkey.set_parameter(get_cur_package_name())
...
|  Ctrl+S(截图)    |  Ctrl+↓(音量减)   |  Ctrl+↑(音量+)  |  Ctrl+C(退出) |
|  F1(帮助)        |  F2(设备信息)      |  F3(刷新设备)    |  F4(wifi-adb) |
|  F5(安装apk)     |  F6(结束当前应用)  |  F7(卸载当前应用)|  F8(重启应用) |
| ---------------------------F10 Monkey测试------------------------------- |'''
