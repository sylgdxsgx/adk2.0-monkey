...
def runnerPool():
    try:
        ...
        t = os.popen("del /F /Q info").read()  # 删除持久化目录
    devices_Pool = []
...
def start(devicess):
    ...
    start_monkey(devices, "adb -s " + devices + " shell " + mc["cmd"], mc["log"])
    time.sleep(1)
    ...
    while True:
        with open(mc["monkey_log"], encoding='utf-8') as monkeylog:
            ...
            BaseMonitor.get_battery(devices)  # 上方的beforeBattery和下方afterBattery会使电量列多两行,后面再优化
...
# 开始脚本测试
def start_monkey(device, cmd, log):
    ...
    cmd2 = "adb -s %s logcat -d >%s" % (device, logcatname)
    os.popen(cmd2)

    # "导出traces文件"
    tracesname = log + r"traces.log"
    cmd3 = "adb  -s %s shell cat /data/anr/traces.txt>%s" % (device, tracesname)
    os.popen(cmd3)
